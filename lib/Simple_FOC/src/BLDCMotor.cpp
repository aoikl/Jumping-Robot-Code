...
int BLDCMotor::alignCurrentSense() {
    ...
    if (monitor_port)
        monitor_port->println(F("MOT: Align current sense."));
    ...
    if (!exit_flag) {
        ...
        if (monitor_port)
            monitor_port->println(F("MOT: Align error!"));
        exit_flag = 0;
    } else {
        // output the alignment status flag
        if (monitor_port)
            monitor_port->print(F("MOT: Success: "));
        if (monitor_port)
            monitor_port->println(exit_flag);
    }
...
int BLDCMotor::alignSensor() {
    ...
    if (!_isset(sensor_direction)) {
        ...
        if (mid_angle == end_angle) {
            if (monitor_port)
                monitor_port->println(F("MOT: Failed to notice movement"));
            return 0;  // failed calibration
        } else if (mid_angle < end_angle) {
            if (monitor_port)
                monitor_port->println(F("MOT: sensor_direction==CCW"));
            sensor_direction = Direction::CCW;
        } else {
            if (monitor_port)
                monitor_port->println(F("MOT: sensor_direction==CW"));
            sensor_direction = Direction::CW;
        }
        // check pole pair number
        if (monitor_port)
            monitor_port->print(F("MOT: PP check: "));
        float moved = fabs(mid_angle - end_angle);
        if (fabs(moved * pole_pairs - _2PI) > 0.5) {  // 0.5 is arbitrary number it can be lower or higher!
            if (monitor_port)
                monitor_port->print(F("fail - estimated pp:"));
            if (monitor_port)
                monitor_port->println(_2PI / moved, 4);
        } else if (monitor_port)
...
void BLDCMotor::loopFOC() {
    ...
    switch (torque_controller) {
        ...
        default:
            ...
            if (monitor_port)
                monitor_port->println(F("MOT: no torque control selected!"));
            break;
    }
